...
4. 이름, 소속, 직함, 담당, 내선번호, 개인번호로 검색하거나 `행정부서 / 학과 / 기타시설`을 따라 조회합니다.
...
- `연락처 파일 만들기`를 누르면 iOS에서는 먼저 `text/vcard` 데이터 URL을 새 브라우징 컨텍스트로 열어 시스템 연락처 카드/가져오기 화면 진입을 시도합니다. Dials 화면은 뒤에 그대로 유지됩니다.
- 직접 열기가 브라우저에서 차단되면 자동으로 기존 `text/vcard` Web Share로 넘어가고, 공유도 사용할 수 없으면 일반 `.vcf` 다운로드로 돌아갑니다.
- 직접 열기 자체는 성공했지만 해당 iOS 브라우저/PWA가 연락처 화면으로 넘기지 못하는 경우를 위해 `공유로 다시 시도` 버튼을 제공합니다.
...
Dials v0.7.3는 **HJU Phonebook V0.26.5 build76**이 생성하는 `.dials` schema **1.4만** 지원합니다. 아직 배포 전이므로 1.3/1.2/1.1 호환 분기는 의도적으로 제거했습니다.
...
Dials v0.7.3 supports only `.dials` schema **1.4** generated by **HJU Phonebook V0.26.5 build76**. Because the project is still pre-deployment, legacy schema fallbacks were intentionally removed.
